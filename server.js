...
// --- 1. PENANGANAN FOLDER OTOMATIS (Penting agar user tidak error) ---
const baseDir = process.cwd(); 
const uploadDir = path.join(baseDir, 'public', 'uploads'); // Simpan di dalam public
...
const storage = multer.diskStorage({
    destination: function (req, file, cb) {
        cb(null, uploadDir); // Menggunakan variabel uploadDir yang sudah kamu buat di baris 26
    },
...
// Ganti baris app.use('/uploads', ...) kamu dengan ini:
...
const dbPath = path.join(dbDir, 'tatriz.db');
const db = new sqlite3.Database(dbPath, (err) => {
    if (err) console.error("❌ Error DB:", err.message);
    console.log('✅ Terhubung ke database di:', dbPath);
});
...
function isAdmin(req, res, next) {
    // Tambahkan pengecekan req.session && ...
    if (req.session && req.session.userId && req.session.role === 'admin') {
        return next();
    } else {
        // Jangan pakai res.status(403).send jika ingin otomatis ke login
        return res.send("<script>alert('Sesi habis atau bukan Admin. Silakan Login.'); window.location='/';</script>");
    }
}


// --- 4. GLOBAL DATA MIDDLEWARE (Versi Aman untuk User Baru) ---
// --- 4. GLOBAL DATA MIDDLEWARE (GANTI SEMUA ISI DI SINI) ---
app.use((req, res, next) => {
    // 1. Siapkan data default (Penting: Agar user baru tidak error)
    res.locals.config = { 
    ...
    // 2. Jika belum login, langsung lanjut (Render login page aman)
    if (!tId) {
        return next();
    }
    ...
    // 3. Ambil data settings tenant (Logo & Nama Toko)
    db.get("SELECT * FROM settings WHERE tenant_id = ?", [tId], (err, row) => {
        if (row) {
            // Jika ada data di DB, gunakan itu. Jika logo_path kosong, kasih default.png
            res.locals.config = row;
            if (!res.locals.config.logo_path) {
                res.locals.config.logo_path = "default.png";
            }
        }

        // 4. Hitung Saldo & Uang Dikunci (Biar dashboard & laporan kas tidak kosong)
        ...
        db.get(sqlSaldo, [tId], (err, s) => {
            db.get(sqlBeban, [bulanIni + '%', tId], (err, b) => {
                ...
                const configData = res.locals.config;
                ...
                // 5. Akhirnya, lanjut ke halaman yang dituju
                next(); 
            });
        });
    });
...
// 1. Tampilkan halaman Form Registrasi
app.get('/register', (req, res) => {
    res.render('register'); // Ini akan mencari file views/register.ejs
});
...
app.post('/save-settings-all', isAdmin, upload.single('logo'), (req, res) => {
    const tId = req.session.tenantId;
    if (!tId) return res.redirect('/login');
    ...
    // 1. Ambil nama file baru jika ada
    let logoPath = req.file ? req.file.filename : null;

    db.serialize(() => {
        // Gunakan COALESCE atau IFNULL di SQL untuk menjaga data lama
        let sqlSettings = `UPDATE settings SET 
        ...
        // Pastikan angka 0 tetap terhitung sebagai angka, bukan null
        let params = [
            nama_perusahaan, alamat, no_hp, 
        ...
        if (logoPath) {
            sqlSettings += `, logo_path = ?`;
            params.push(logoPath);
        }

        sqlSettings += ` WHERE tenant_id = ?`;
        params.push(tId);

        // 2. Jalankan dengan penanganan error sederhana
        db.run(sqlSettings, params, (err) => {
            if (err) console.error("❌ Gagal update settings:", err.message);
        });

        // 3. Tambah mesin baru
        if (nama_mesin_baru && nama_mesin_baru.trim() !== "") {
            db.run(`INSERT INTO mesin (tenant_id, nama_mesin) VALUES (?, ?)`, 
            [tId, nama_mesin_baru.trim()], (err) => {
                if (err) console.error("❌ Gagal tambah mesin:", err.message);
            });
        }
    });

    // Gunakan redirect dengan sedikit jeda atau langsung
    res.redirect('/setup');
});
...
app.post('/save-po', isAdmin, (req, res) => {
    const tId = req.session.tenantId;
    ...
    // Pastikan input rincian diproses sebagai Array
    ...
    db.serialize(() => {
        // 1. Simpan Header PO
        db.run(`INSERT INTO po_utama (tenant_id, tanggal, nama_po, customer, status) VALUES (?, ?, ?, ?, ?)`, 
        [tId, tanggal, nama_po, customer, status], function(err) {
            if (err) return res.status(500).send("Gagal simpan PO Utama");

            const poId = this.lastID; // Ambil ID PO yang baru saja dibuat
            let totalTagihan = 0;

            // 2. Simpan Rincian Detail (Looping)
            for (let i = 0; i < jbList.length; i++) {
                ...
                // Logika Level: Jika Level 1, harga operator dipaksa ikut harga customer
                let finalHOp = (tId !== 1 && tLevel < 2) ? hCu : (Number(hrgOpList[i]) || 0);
                
                totalTagihan += (qty * hCu);

                db.run(`INSERT INTO po_detail (po_id, jenis_bordir, nama_desain, jumlah, harga_operator, harga_customer, harga_cmt) 
                        VALUES (?, ?, ?, ?, ?, ?, 0)`, 
                [poId, jbList[i], dsList[i], qty, finalHOp, hCu]);
            }
...
app.post('/admin/simpan-kerja', (req, res) => {
    // 1. Ambil data dari form
    const { 
        user_id_manual, // Dari form Admin
        tanggal, 
        shift, 
        po_id, 
        detail_id, 
    ...
    // 2. LOGIKA PENENTU SIAPA YANG BEKERJA
    // Jika ada user_id_manual (dari Admin), pakai itu. 
    // Jika tidak ada (dari Operator langsung), pakai ID dari session.
    let targetUserId = user_id_manual ? Number(user_id_manual) : Number(req.session.userId);
    ...
    db.serialize(() => {
        // Pastikan nama kolom sesuai dengan database Anda (operator_id)
        const sqlInsert = `INSERT INTO hasil_kerja 
        ...
        db.run(sqlInsert, [
            ...
            if (err) {
                ...
                return res.status(500).send("Gagal simpan data kerja.");
            }

            // AUTO-UPDATE STATUS KE QC JIKA SUDAH SELESAI
            const sqlCheck = `
                SELECT 
            ...
            db.get(sqlCheck, [po_id, po_id], (err, row) => {
                ...
                // Jika yang input Admin, balik ke Dashboard. Jika Operator, balik ke Hasil Saya.
...
app.get('/api/po-details/:id', (req, res) => {
    ...
    db.all(sql, [poId], (err, rows) => {
        if (err) return res.json([]);
        res.json(rows); // Admin bisa melihat semua sisa, termasuk yang sudah 0 untuk koreksi
    });
});
...
app.post('/update-produksi', isAdmin, (req, res) => {
    const tId = req.session.tenantId; // Proteksi Multi-Tenant
    const { 
        id_prod,      // ID unik dari baris hasil_kerja
        tanggal, 
        op,           // Nama Operator (Teks)
        shift, 
        nama_po,      // Update ke po_utama
        nama_bordir,  // Update ke po_detail
        jenis_bordir, // Update ke po_detail
        jumlah,       // Update ke hasil_kerja
        harga         // Update ke po_detail
    } = req.body;

    // Menghitung total harga pabrik (upah) secara otomatis
    const total_h_pabrik = Number(jumlah) * Number(harga);

    db.serialize(() => {
        // 1. Ambil relasi ID agar bisa update tabel induknya
        db.get("SELECT po_id, detail_id FROM hasil_kerja WHERE id = ? AND tenant_id = ?", [id_prod, tId], (err, row) => {
            if (err || !row) {
                console.error("❌ Data tidak ditemukan untuk ID:", id_prod);
            ...
            // 2. UPDATE PO_UTAMA (Fleksibilitas Nama PO)
            db.run("UPDATE po_utama SET nama_po = ? WHERE id = ? AND tenant_id = ?", 
                [nama_po, poId, tId]);

            // 3. UPDATE PO_DETAIL (Fleksibilitas Desain & Harga Upah)
            db.run("UPDATE po_detail SET nama_desain = ?, jenis_bordir = ?, harga_operator = ? WHERE id = ?", 
                [nama_bordir, jenis_bordir, Number(harga), detailId]);

            // 4. UPDATE HASIL_KERJA (Fleksibilitas Log Harian)
            // Catatan: Nama OP di halaman Anda adalah hasil JOIN dari tabel users.
            // Untuk mengganti OP secara permanen, Anda perlu mencari ID user berdasarkan nama 'op'.
            db.run("UPDATE hasil_kerja SET tanggal = ?, shift = ?, jumlah_setor = ? WHERE id = ? AND tenant_id = ?", 
                [tanggal, shift, Number(jumlah), id_prod, tId], (err) => {
                    if (err) {
                        console.error("❌ Gagal Update Fleksibel:", err.message);
                        return res.status(500).send("Gagal memperbarui data.");
                    }
                    
                    console.log(`✅ Koreksi Berhasil: PO ${nama_po} (ID:${id_prod}) diperbarui.`);
...
// Rute Khusus Koreksi di Halaman Monitor (Laporan Produksi)
app.post('/admin/update-koreksi-produksi', isAdmin, (req, res) => {
    const tId = req.session.tenantId;
    const { 
        log_id, 
        tanggal_baru, 
    ...
    // Query untuk update log spesifik
    // Mendukung pindah desain (detail_id_baru) dan perubahan jumlah setor
    const sql = `
        UPDATE hasil_kerja 
        SET tanggal = ?, 
            shift = ?, 
    ...
    db.run(sql, [tanggal_baru, shift_baru, detail_id_baru, jumlah_baru, log_id, tId], (err) => {
        ...
        // Redirect kembali ke halaman monitor agar progres (Sisa/Target) langsung terupdate
...
app.post('/save-kas', isAdmin, (req, res) => {
    const tId = req.session.tenantId;
    const { kas_id, tanggal, jenis, kategori, jumlah, keterangan, po_id } = req.body;
    const ref_po = (kategori === "PEMBAYARAN BORDIR" || kategori === "PELUNASAN" || kategori === "DP/CICILAN") ? po_id : null;
...
// Rute Halaman Input Darurat (Khusus Admin)
app.get('/input-kerja-admin', isAdmin, (req, res) => {
    const tId = req.session.tenantId;

    // 1. Query hanya untuk PO yang statusnya sedang Produksi
    const query1 = "SELECT id, nama_po FROM po_utama WHERE tenant_id = ? AND status = 'Produksi'";
    const query2 = "SELECT id, nama_mesin FROM mesin WHERE tenant_id = ?";
    const query3 = "SELECT id, nama_lengkap FROM users WHERE tenant_id = ? AND role = 'operator'";
    const query4 = "SELECT * FROM settings WHERE tenant_id = ?";

    db.all(query1, [tId], (err, active_pos) => {
        db.all(query2, [tId], (err, daftarMesin) => {
            db.all(query3, [tId], (err, daftarOperator) => {
                db.get(query4, [tId], (err, config) => {
...
// Route khusus untuk mematikan server dari browser
app.get('/exit-app', (req, res) => {
    res.send(`
        <body style="font-family:sans-serif; text-align:center; padding-top:50px; background:#f4f4f4;">
            <div style="background:white; display:inline-block; padding:30px; border-radius:10px; shadow: 0 4px 6px rgba(0,0,0,0.1);">
                <h2>Aplikasi Berhasil Dimatikan</h2>
                <p>Anda sekarang dapat menutup jendela browser ini.</p>
                <button onclick="window.close()" style="padding:10px 20px; cursor:pointer;">Tutup Jendela</button>
            </div>
        </body>
    `);

    console.log("🛑 Menerima perintah shutdown. Menutup server...");
    
    // Memberi jeda 1 detik agar browser sempat menampilkan pesan di atas
    setTimeout(() => {
        process.exit(0); 
    }, 1000);
});
