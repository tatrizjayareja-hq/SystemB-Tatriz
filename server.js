...
app.get('/laporan-kas', isAdmin, async (req, res) => {
    ...
    try {
        ...
        // 1. Query Statistik Keuangan BULAN INI
        const sqlData = `
            SELECT 
                ...
                (SELECT COALESCE(SUM(h.jumlah_setor * d.harga_customer), 0) 
                 ...
                 AND h.tenant_id = $2 
                 AND h.mesin_id::TEXT = '20') as prod_mesin_22_bln,
        ...
        // 🌟 2. Query Tabungan Profit AKUMULASI (ALL-TIME) 
        const sqlAkumulasi = `
            SELECT 
        ...
        const akData = akumulasiRes.rows[0];

        // Kalkulasi All-Time Retained Earnings
        const profitKotorAllTime = (parseFloat(akData.prod_all) - parseFloat(akData.op_all)) + (parseFloat(akData.omzet_cmt_all) - parseFloat(akData.hpp_cmt_all)) - parseFloat(akData.kontrakan_all);
        const sisaProfitAkumulasi = profitKotorAllTime - parseFloat(akData.ditarik_all);
...
app.get('/laporan-kas-v2', isAdmin, async (req, res) => {
    const tId = req.session.tenantId;
    const isPro = (tId === 1 || tId === 100 || req.session.tenantLevel >= 2);
...
app.get('/kiosk-produksi', async (req, res) => {
    ...
    try {
        ...
        // 2. QUERY DETAIL (Sama Persis dengan po-data-v2)
        const sqlDetails = `
            SELECT 
                d.*, 
                (SELECT COALESCE(SUM(h.jumlah_setor), 0) FROM hasil_kerja h WHERE h.detail_id = d.id) as total_produksi
        ...
        res.status(500).send("Gagal memuat Kiosk Produksi: " + err.message);
    }
});
...
app.get('/cetak-nota-vendor', isAdmin, async (req, res) => {
    ...
    try {
        ...
        console.error(err);
        res.status(500).send("Gagal cetak nota");
    }
});
...
app.post('/save-kas', isAdmin, async (req, res) => {
    ...
    try {
        ...
        // ==========================================
        // 🔴 JALUR 3: UPDATE STATUS VENDOR JIKA BAYAR CMT
        // ==========================================
...
// FUNGSI HELPER AGAR STATUS PO BERUBAH MENJADI DP/LUNAS
async function updateStatusPOBawaan(poId, tenantId, dbInstance) {
    try {
        const checkPO = await dbInstance.query("SELECT status, total_harga_customer FROM po_utama WHERE id = $1 AND tenant_id = $2", [poId, tenantId]);
        const poLama = checkPO.rows[0];
        if (!poLama) return;
